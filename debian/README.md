...
2. Install Debian to hardware (Optional)
    ...
    3. Create a bootable usb from iso
       ...
       ```shell
       sudo cp ./installation/build/output/preseed-debian.iso /dev/disk/by-id/<usb-id>
       sudo sync
       ```
       The whole device must be selected, not the partition!
       
       **The device provided will be overwritten, use with care! All data will be lost!**
    5. Install the given Linux OS onto the hardware by booting from the usb
